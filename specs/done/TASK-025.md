...
- **文件**: `backend/storage/session_store.py` 第 128 行
...
- **文件**: `backend/runtime/llm_client.py` 第 29-33 行
...
- **文件**: `backend/runtime/services.py` 第 206 行 + 第 150 行
...
- **文件**: `backend/storage/session_store.py` 第 59-62 行
...
- **文件**: `backend/tools_defs/fs_search.py` 第 17 行
- **问题**: `rglob("*")` 递归遍历整个目录树，模型如果传 `/` 或大目录会 OOM 或长时间卡住。
- **修法**: 加 `max_depth` 参数（默认 3 层）和 `max_results` 参数（默认 50 条），超限即停。

### B6 🟡 文件工具无路径限制（安全隐患）
- **文件**: `backend/tools_defs/fs_write.py` / `fs_read.py`
- **问题**: `write_file` / `read_file` 可以读写任意路径，模型可能意外操作系统文件。
- **修法**: 引入 `allowed_roots` 白名单或 `working_directory` 沙箱，默认只允许操作项目目录内的文件。

## Done 条件
- [x] B1: `delete()` 去掉 commit，service 层统一事务
- [x] B2: `llm_client` 错误处理修正
- [x] B3: 自动 compact 路径不再提前 commit
- [x] B4: upsert 字段覆写改为区分“未传”和“显式传 None”
- [ ] B5: `search_text` 加深度和结果数量限制
- [ ] B6: 文件工具加路径白名单限制
- [x] 所有已完成改动通过 `python3 -m unittest backend.tests.test_agent -v`

## 收口说明
- 本任务主链路收口到 B1-B4：已完成删除事务边界、LLM HTTP/响应结构错误处理、自动 compact 不提前落库、session nullable 元数据哨兵更新。
- B5/B6 不再阻塞当前任务收口，后续并入工具专项统一处理：一起解决搜索深度/结果限制、文件读写白名单，以及后续新增工具能力的边界治理。
