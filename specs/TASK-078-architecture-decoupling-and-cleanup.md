...
- [ ] **切片 1：L1 模型层依赖纠偏与 L2 提示词层无状态解耦**
  - [ ] 修改 `agent_prototype/model/adapters/chat_completions.py`，直接从 `agent_prototype.model.types.domain` 导入 `ChatMessage`，消除对 API DTO 的越权依赖。
  - [ ] 重构 `agent_prototype/prompt/builder.py` 的 `build_runtime_system_prompt`：
  ...
  - [ ] **⚠️ 同步修改 `agent_prototype/context/skill_context.py`**（此文件调用了 `build_runtime_system_prompt(..., workspace_path=workspace_path)`，切片 1 完成后此处直接编译报错，必须一并处理）：
...
- [ ] **切片 2：L6 统一上下文装配器（ContextAssembler）落地与 L5 记忆压缩去推理化**
  - [ ] 在 `agent_prototype/context/assembler.py` 中落地 `ContextAssembler` [NEW]：
  ...
  - [ ] 在 `agent_prototype/context/compaction.py` 中新增 `HistoryCompactor` 类（⚠️ 接口约定如下，落地时不得偏离）：
    ```python
    ...
    ```
  - [ ] 重构 `agent_prototype/memory/summary/service.py` 内部的 `CompactService`（⚠️ 此处有两条并行违规，须同步修复）：
    ...
    - **违规 B（L5→L8）**：第 34 行 `from agent_prototype.execution.persistence.run_context_builder import RunContextBuilder` — L5 记忆层直接引用 L8 执行层的 `RunContextBuilder` 来构建 LLM 请求上下文，与违规 A 同根（LLM 请求构建逻辑不应在 L5 内部），随违规 A 一并剥出；`CompactService` 完成修复后不再持有任何 L8 依赖。
    - 修复后 `CompactService` 只负责 Snapshot 的持久化状态存取与 Run 活跃标志位修改。
  - [ ] 运行测试确认历史管理与压缩链路完全无损。

- [ ] **切片 3：L3 子智能体派发工具与 L8 执行层“三纯回调”依赖倒置解耦**
  - [ ] 重构 `agent_prototype/tools/builtin/agent_bridge/spawn_child_agent.py`：
  ...
  - [ ] 重构 `agent_prototype/tools/builtin/agent_bridge/check_child_status.py`：
    - 彻底移出对 Python `concurrent.futures.Future` 对象的直接操作与 `futures` 字典导入；
    - 改造为接收状态查询器回调 `status_checker: Callable[[list[str]], dict[str, Any]]`。
  - [ ] 重构 `agent_prototype/tools/builtin/agent_bridge/wait_child_agent.py`：
    - 彻底移出对 `f.result` 的直接操作与超时等待逻辑（超时逻辑一并移至 L8）；
    - 改造为接收阻塞等待器回调 `child_waiter: Callable[[str], str]`。
  - [ ] 重构 `agent_prototype/tools/registry.py` 中的 `build_run_registry` 签名，移除 `executor`/`futures` 参数：
    ```python
    ...
    ```
  - [ ] 在 `agent_prototype/execution/persistence/run_service.py` 内部实现并注入三个回调闭包：
...
- [ ] **切片 4：全量测试与向后兼容保障**
  - [ ] 调整 `agent_prototype/execution/persistence/run_context_builder.py`：
